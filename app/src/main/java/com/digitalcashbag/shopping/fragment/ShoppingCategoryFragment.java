...
import com.digitalcashbag.shopping.adapter.ShoppingFragmentAdapter;
...
public class ShoppingCategoryFragment extends BaseFragment implements MvpView {
    Unbinder unbinder;
    @BindView(R.id.gv)
    RecyclerView gv;

    private List<CategoryGroupWiseItem> itemList;
    ...
    public void onViewCreatedStuff(View view, @Nullable Bundle savedInstanceState) {
        try {
            ...
            gv.setLayoutManager(new GridLayoutManager(getContext(), 3));
    ...
    private void getShoppingCategory() {
        ...
        object.addProperty("FK_CategoryId", "");
        object.addProperty("Parameter", "0");
            ...
            public void onResponse(@NonNull Call<ResponseMainCategory> call, @NonNull Response<ResponseMainCategory> response) {
                ...
                try {
                    ...
                    if (response.body() != null && response.body().getResponse().equalsIgnoreCase("Success") &&
                            ...
                            response.body().getCategoryData().get(0).getCategoryGroupWise().size() > 0) {
                        itemList = response.body().getCategoryData().get(0).getCategoryGroupWise();
                        ShoppingFragmentAdapter shoppingFragmentAdapter =
                                new ShoppingFragmentAdapter(context, itemList, ShoppingCategoryFragment.this, true);
    ...
    public void getClickPosition(int id, String name) {
        if (NetworkUtils.getConnectivityStatus(context) != 0) {
            getShoppingCategory(String.valueOf(id), name);
        } else {
    ...
    private void getShoppingCategory(String category_id, String name) {
        showLoading();
        JsonObject object = new JsonObject();
        object.addProperty("PK_CategoryId", "");
        ...
        responseCategoryCall.enqueue(new Callback<ResponseMainCategory>() {
            ...
            public void onResponse(@NonNull Call<ResponseMainCategory> call, @NonNull Response<ResponseMainCategory> response) {
                ...
                if (response.body() != null && response.body().getResponse().equalsIgnoreCase("Success")
                    ...
                    ((ShoppingActivityMain) getActivity()).ReplaceFragment_AddBack(new ShoppingSubCategoryFragment(), name, Integer.parseInt(category_id));
                } else {
                    if (NetworkUtils.getConnectivityStatus(context) != 0) {
                        ((ShoppingActivityMain) getActivity()).ReplaceFragment_AddBack(new ShoppingProductListFragment(), name, Integer.parseInt(category_id));
                    } else {
                        showAlert(getResources().getString(R.string.alert_internet), R.color.red, R.drawable.error);
                    }
                }
    ...
    public void onResume() {
        super.onResume();
        ((ShoppingActivityMain) context).title.setText("Shopping");
        if (NetworkUtils.getConnectivityStatus(context) != 0) {
            getShoppingCategory();
        } else {
            showAlert(getResources().getString(R.string.alert_internet), R.color.red, R.drawable.error);
