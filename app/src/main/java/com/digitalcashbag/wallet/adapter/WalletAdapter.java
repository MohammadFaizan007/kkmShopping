...
public class WalletAdapter extends RecyclerView.Adapter<WalletAdapter.ViewHolder> implements Filterable {

    private final Context context;


    private UserFilter userFilter;
    private List<AllTransactionListItem> list;
    private MvpView mvpView;

    public WalletAdapter(Context context, List<AllTransactionListItem> list, MvpView mvpView) {
        this.context = context;
        this.list = list;
        this.mvpView = mvpView;
    }

    public void updateList(List<AllTransactionListItem> list) {
        this.list = list;
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        holder.transDate.setText(list.get(position).getPaymentDate());

        if (list.get(position).getTransactionId().equalsIgnoreCase("") ||
                list.get(position).getTransactionId().equalsIgnoreCase("NA")) {
            holder.transactionId.setVisibility(View.GONE);
            holder.transactionId.setText("");
        } else {
            holder.transactionId.setVisibility(View.VISIBLE);
            holder.transactionId.setText(String.format(list.get(position).getTransactionId()));
        }


        if (list.get(position).getRemarks().equalsIgnoreCase("") ||
                list.get(position).getRemarks().equalsIgnoreCase("NA")) {
            holder.remark.setVisibility(View.INVISIBLE);
            holder.remark.setText("");
        } else {
            holder.remark.setVisibility(View.VISIBLE);
            holder.remark.setText(String.format(list.get(position).getRemarks().trim()));
        }

        holder.mobile.setText(list.get(position).getMobileNo());
        holder.operatorName.setText(String.format(" (%s)", list.get(position).getOperator()));

        if (list.get(position).getCrAmount().equalsIgnoreCase("0.00")) {
            holder.amount.setText(String.format("- ₹ %s", list.get(position).getDrAmount()));
            holder.amount.setTextColor(ContextCompat.getColor(context, R.color.red));
        } else if (list.get(position).getDrAmount().equalsIgnoreCase("0.00")) {
            holder.amount.setText(String.format("+ ₹ %s", list.get(position).getCrAmount()));
    ...
    private static class UserFilter extends Filter {
        ...
        List<AllTransactionListItem> myDirectResponses;
        List<AllTransactionListItem> filteredList;

        private UserFilter(WalletAdapter adapter, List<AllTransactionListItem> originalList) {
            super();
        ...
        protected FilterResults performFiltering(CharSequence constraint) {
            ...
            if (constraint.length() == 0) {
                ...
                for (final AllTransactionListItem user : myDirectResponses) {
        ...
        protected void publishResults(CharSequence constraint, FilterResults results) {
            adapter.list.clear();
            adapter.list.addAll((ArrayList<AllTransactionListItem>) results.values);
